...
void CSubclassWnd::AssertValid() const
{
	...
	if (m_hWnd) {
      CSubclassWnd* p = NULL; //skst: enforce for scoping
...
void CSubclassWndMap::Add(HWND hwnd, CSubclassWnd* pSubclassWnd)
{
	...
	if (pSubclassWnd->m_pNext==NULL) {
		...
		pSubclassWnd->m_pOldWndProc = 
         (WNDPROC) ::SetWindowLongPtr(hwnd, GCLP_WNDPROC, (LONG_PTR) HookWndProc);  //skst: use appropriate function
...
void CSubclassWndMap::Remove(CSubclassWnd* pUnHook)
{
	...
	if (pHook==pUnHook) {
		...
		if (pHook->m_pNext)
			...
			RemoveKey(hwnd);
         ::SetWindowLongPtr(hwnd, GCLP_WNDPROC, (LONG_PTR) pHook->m_pOldWndProc);   //skst: use appropriate function
		}
	} else {
